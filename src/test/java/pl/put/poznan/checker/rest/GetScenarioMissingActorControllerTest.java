...
public class GetScenarioMissingActorControllerTest {
    ...
    public void testGetScenarioMissingActor() {
        ...
        GetScenarioMissingActorController controller = new GetScenarioMissingActorController(storage,logger);

        List<Step> result = controller.getScenarioMissingActor(title);
        List<Step> noActorSteps = Arrays.asList(step0, step1_2, step2_2);
        assertEquals(noActorSteps, result);
    }
}
